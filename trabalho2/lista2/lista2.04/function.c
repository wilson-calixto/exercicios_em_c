...
{
	float x1, x2, delta; //declara as variaveis
	...
	else
	{
		...
		else if(delta == 0) //se delta for igual a zero x1 sera igual a x2, sendo assim
		{
			...
			//a partir daqui, sera usada a funcao <sqrt> (que calcula a raiz quadrada) inclusa na biblioteca <math.h>

			x1 = (-b + sqrt(delta))/(2*a); //formula para x1 (x linha) 

			printf("x1 e x2 = %.2f \n", x1);
		}
		else
		{
			x1 = (-b + sqrt(delta))/(2*a); //formula para x1
			x2 = (-b - sqrt(delta))/(2*a); //formula para x2
