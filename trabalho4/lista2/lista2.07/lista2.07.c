...
int main()
{
	float *vetor,*nvetor,procurado;
	int i,e,indice,loops,vezes_q_apareceu,re;
	
	/* pede o numero de loops */
	printf("digite os loops: ");    	
	scanf("%d",&loops);
	vetor = (float*) ( malloc( sizeof(float) * loops ) );//vetor[loops]
	nvetor = (float*) ( malloc( sizeof(float) * loops ) );//vetor[loops]
	/* carrega os numeros no vetor */
	for (i=0;i<loops;i++ )
	{
		printf ("Digite o numero %d: ",i+1);
		scanf ("%f", &vetor[i]);
		nvetor[i]=388888888888888;
	}
	...
	for (i=0;i<loops ;i++ )
	{
		...
		procurado=vetor[i];
		re=qnt_ap(nvetor,loops,procurado);
		...
		if(re==0)
		{
			...
			{
				printf ( "o numero %.2f ocorre : %d vez\n",vetor[i],vezes_q_apareceu );
			}
			else
			{
				printf ( "o numero %.2f ocorreu : %d vezes\n",vetor[i],vezes_q_apareceu );
			}
